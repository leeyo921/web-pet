...
| `home-x` | 初始横向落点，百分比（`"56%"`，相对视口宽）或像素（`"420"`）。默认右下角 |
| `home-y` | 初始纵向落点，同上。这是**姿态底边的基线**，也就是宠物脚踩的那条线 |

`home-x` / `home-y` 只决定初始落点；之后宠物会自己游荡，也可以被拖走。写错格式会
回退到默认的右下角，宿主页面改坏了不至于把宠物摆到视口外面。注意它对移动端同样
生效——移动端版式差异大时，宿主自行按 `innerWidth` 决定要不要加这两个属性。
